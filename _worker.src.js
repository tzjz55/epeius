...
function 配置信息(UUID, 域名地址) {
	...
	const 传输层协议 = 'WS'.toLocaleLowerCase();
	const 伪装域名 = 域名地址;
	const 路径 = '/?ed=2560';
	...
	const v2ray = `${协议类型}://${用户ID}@${地址}:${端口}?encryption=${加密方式}&security=${传输层安全[0]}&${'SIN'.toLocaleLowerCase()}=${SNI}&fp=${指纹}&type=${传输层协议}&host=${伪装域名}&path=${encodeURIComponent(路径)}#${encodeURIComponent(别名)}`;
...
function subAddresses(host,UUID,noTLS,newAddressesapi,newAddressescsv,newAddressesnotlsapi,newAddressesnotlscsv) {
	...
	if (noTLS == 'true'){
		...
		notlsresponseBody = uniqueAddressesnotls.map(address => {
			...
			const vlessLink = `${协议类型}://${UUID}@${address}:${port}?encryption=none&security=&type=${'WS'.toLocaleLowerCase()}&host=${伪装域名}&path=${encodeURIComponent(最终路径)}#${encodeURIComponent(addressid + 节点备注)}`;
	...
	const responseBody = uniqueAddresses.map(address => {
		...
		const 协议类型 = atob(啥啥啥_写的这是啥啊);
		const vlessLink = `${协议类型}://${UUID}@${address}:${port}?encryption=none&security=tls&${'SIN'.toLocaleLowerCase()}=${伪装域名}&fp=random&type=${'WS'.toLocaleLowerCase()}&host=${伪装域名}&path=${encodeURIComponent(最终路径)}#${encodeURIComponent(addressid + 节点备注)}`;
